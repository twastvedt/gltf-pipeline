            ...
            if (featureTables.hasOwnProperty(featureTableName)) {
                ...
                for (const propertyName in properties) {
                    if (properties.hasOwnProperty(propertyName)) {
                        ...
                        const offsetBufferViews = property.offsetBufferViews;
                        if (defined(offsetBufferViews)) {
                            const offsetBufferViewsLength = offsetBufferViews.length;
                            for (let i = 0; i < offsetBufferViewsLength; i++) {
...
Remove.texture = function(gltf, textureId) {
    ...
    if (hasExtension(gltf, 'EXT_feature_metadata')) {
        ForEach.mesh(gltf, function(mesh) {
            ForEach.meshPrimitive(mesh, function(primitive) {
                if (defined(primitive.extensions) &&
                    ...
                    const featureIdTextures = primitive.extension.EXT_feature_metadata;
                    ...
                    for (let i = 0; i < featureIdTexturesLength; ++i) {
                        ...
                        const textureAccessor = featureIdTexture.featureIds;
                        const textureInfo = textureAccessor.texture;
                        if (textureInfo.index > textureId) {
                            --textureInfo.index;
                        }
                    }
        ...
        for (const featureTextureName in featureTextures) {
            if (featureTextures.hasOwnProperty(featureTextureName)) {
                const featureTexture = featureTextures[featureTextureName];
